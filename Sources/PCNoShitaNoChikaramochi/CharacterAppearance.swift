...
final class AppearanceStore: ObservableObject {
    ...
    private static let keyGender = "appearance.gender"
    private static let keySkin = "appearance.skin"
    private static let keyShirt = "appearance.shirt"

    init() {
        let d = UserDefaults.standard
        self.gender = Gender(rawValue: d.string(forKey: Self.keyGender) ?? "") ?? .male
        self.skinTone = SkinTone(rawValue: d.string(forKey: Self.keySkin) ?? "") ?? .light
        self.shirtColor = ShirtColor(rawValue: d.string(forKey: Self.keyShirt) ?? "") ?? .red
    }
    ...
    private func persist() {
        let d = UserDefaults.standard
        d.set(gender.rawValue, forKey: Self.keyGender)
        d.set(skinTone.rawValue, forKey: Self.keySkin)
        d.set(shirtColor.rawValue, forKey: Self.keyShirt)
    }
}
